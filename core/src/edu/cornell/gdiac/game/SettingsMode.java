...
//    }
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        if (backButton == null || backButtonState == 2) {
            return true;
        }
        ...
        // TODO: Fix scaling
        // Play button is a circle. no it is not
        float width = BUTTON_SCALE*scale*backButton.getWidth()/2.0f;
        float height = BUTTON_SCALE*scale*backButton.getHeight()/2.0f;
        float distW = (screenX-centerX*3f/2f)*(screenX-centerX*3f/2f);
        float distH = (screenY-centerY*3f/2f)*(screenY-centerY*3f/2f);
        if (distW < width*width && distH < height*height) {
            backButtonState = 1;
        }
        return false;
    }
