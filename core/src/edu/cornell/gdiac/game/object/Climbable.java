...
public class Climbable {
    ...
    public Climbable(JsonValue data, int tileSize, int levelWidth, int levelHeight, TextureRegion tileset, Rectangle bounds, int fid, Vector2 textureScale) {

        levelClimbables = data.get("data").asIntArray();
    ...
    public void draw(GameCanvas canvas){
        ...
        for (float y = levelHeight - 1; y >= 0; y--){
            for (float x = 0; x < levelWidth; x++){
                if (levelClimbables[i] > 0) {
                    TextureRegion tileTexture = tileset[levelClimbables[i] - fid];
